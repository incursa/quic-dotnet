...
var adaptiveBackpressureAxisSelected =
    adaptiveBackpressurePolicy.ForcedValue is not null
    || adaptiveBackpressurePolicy.ObservationMode
        != QuicAdaptiveBackpressureObservationMode.Disabled;
var packetFlushCadenceAxisSelected =
...
var bufferObservationMode = adaptiveInstrumentationEnabled
    ? EnableUnifiedBufferCopyObservation(
        bufferCopyPolicy.ObservationMode)
    : bufferCopyPolicy.ObservationMode;
var adaptiveBackpressureObservationMode =
    adaptiveInstrumentationEnabled
        ? EnableUnifiedAdaptiveBackpressureObservation(
            adaptiveBackpressurePolicy.ObservationMode)
        : adaptiveBackpressurePolicy.ObservationMode;
var packetFlushCadenceObservationMode =
    adaptiveInstrumentationEnabled
    ...
    configuredAdaptiveBackpressurePolicy =
        adaptiveInstrumentationEnabled
            ? QuicAdaptiveBackpressurePolicy.CreateConfiguredSnapshot(
                ...
                adaptiveBackpressurePolicy.ForcedValue)
            : null;
    ...
    ConnectionOptionsCallback = (_, _, _) =>
    {
        ...
        return ValueTask.FromResult(new QuicServerConnectionOptions
        {
            ...
            AdaptiveBackpressureEvidenceSink =
                adaptiveBackpressureObservationMode
                    != QuicAdaptiveBackpressureObservationMode.Disabled
                ? connectionSinks?.AdaptiveBackpressureEvidenceSink
                : null,
...
{
    Console.WriteLine(
        $"QUIC_ADAPTIVE_RUNTIME_EVIDENCE_MODE={evidenceMode.ToContractValue()}");
    if (evidenceMode == AdaptiveRuntimeEvidenceMode.BoundedAggregate)
    {
        Console.WriteLine(
...
if (evidenceMode == AdaptiveRuntimeEvidenceMode.Raw)
{
...
static AdaptiveRuntimeEvidenceMode ResolveAdaptiveRuntimeEvidenceMode(
    string? value)
{
    return value?.Trim().ToLowerInvariant() switch
    {
        null or "" or "raw" => AdaptiveRuntimeEvidenceMode.Raw,
        "bounded_aggregate" =>
            AdaptiveRuntimeEvidenceMode.BoundedAggregate,
        _ => throw new InvalidOperationException(
            "PROTOCOL_LAB_INCURSA_RAW_QUIC_EVIDENCE_MODE must be unset, raw, or bounded_aggregate."),
    };
}
...
static (
    ...
    QuicAdaptiveBackpressureObservationMode ObservationMode)
    ResolveAdaptiveBackpressurePolicy(string? value)
{
    return value?.Trim().ToLowerInvariant() switch
    {
        null or "" => (
        ...
        _ => throw new InvalidOperationException(
            "PROTOCOL_LAB_INCURSA_RAW_QUIC_ADAPTIVE_BACKPRESSURE_POLICY must be unset, legacy_current, early_delay, observe_only, or shadow."),
    };
}
...
static (
    ...
    ResolvePacketFlushCadencePolicy(string? value)
{
    return value?.Trim().ToLowerInvariant() switch
    {
        null or "" => (
        ...
        _ => throw new InvalidOperationException(
            "PROTOCOL_LAB_INCURSA_RAW_QUIC_PACKET_FLUSH_CADENCE_POLICY must be unset, legacy_current, prompt, observe_only, or shadow."),
    };
}
...
static QuicAdaptiveBackpressureObservationMode
    EnableUnifiedAdaptiveBackpressureObservation(
        QuicAdaptiveBackpressureObservationMode mode)
    => mode == QuicAdaptiveBackpressureObservationMode.Disabled
        ? QuicAdaptiveBackpressureObservationMode.Shadow
        : mode;

static QuicPacketFlushCadenceObservationMode
    EnableUnifiedPacketFlushCadenceObservation(
...
internal sealed class AdaptiveRuntimeEpochPublisher
{
    ...
    private const string BoundedAggregateEpochOutputPrefix =
    ...
    private const string AdaptiveBackpressureOutputPrefix =
    ...
    private readonly Channel<AdaptiveBackpressureEvidenceRecord>
        adaptiveBackpressureObservations =
            CreateEvidenceChannel<AdaptiveBackpressureEvidenceRecord>();
        ...
        AdaptiveRuntimeEvidenceMode evidenceMode)
    {
        ...
        this.configuredBufferCopyPolicy =
            configuredBufferCopyPolicy;
        this.configuredAdaptiveBackpressurePolicy =
            configuredAdaptiveBackpressurePolicy;
    ...
    private async Task WriteBoundedAggregateEpochsAsync()
    {
        try
        {
            ...
            while (await timer.WaitForNextTickAsync())
            {
                ...
                Console.WriteLine(
                    ...
                    + JsonSerializer.Serialize(
                        epoch,
                        AdaptiveRuntimeEpochJsonContext.Default
                            .BoundedAdaptiveRuntimeEpochRecord));
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task WriteAdaptiveBackpressureObservationsAsync()
    {
        try
        {
            await foreach (AdaptiveBackpressureEvidenceRecord observation
                in adaptiveBackpressureObservations.Reader.ReadAllAsync())
            {
                Console.WriteLine(
                    AdaptiveBackpressureOutputPrefix
                    + JsonSerializer.Serialize(
                        observation,
                        AdaptiveRuntimeEpochJsonContext.Default
                            .AdaptiveBackpressureEvidenceRecord));
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine(
                $"IncursaRawQuicServer adaptive-backpressure evidence writer stopped: {ex.Message}");
        }
    }

    private async Task WritePacketFlushCadenceObservationsAsync()
    {
        try
        {
            await foreach (PacketFlushCadenceEvidenceRecord observation
                in packetFlushCadenceObservations.Reader.ReadAllAsync())
            {
                Console.WriteLine(
                    PacketFlushCadenceOutputPrefix
                    + JsonSerializer.Serialize(
                        observation,
                        AdaptiveRuntimeEpochJsonContext.Default
                            .PacketFlushCadenceEvidenceRecord));
            }
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine(
                $"IncursaRawQuicServer packet-flush cadence evidence writer stopped: {ex.Message}");
        }
    }
        ...
        IQuicAdaptiveRuntimeUnifiedEpochEvidenceSink
    {
        ...
        public bool TryPublish(in QuicApplicationSendTurnPolicyProvenance provenance)
            => owner.evidenceMode
                == AdaptiveRuntimeEvidenceMode.BoundedAggregate
                || owner.applicationSendTurnProvenance.Writer.TryWrite(
                    new ApplicationSendTurnPolicyProvenanceRecord(
            ...
            in QuicAdaptiveBackpressureObservation observation)
        {
            ...
            bool rawPublished =
                owner.adaptiveBackpressureObservations.Writer.TryWrite(
                    new AdaptiveBackpressureEvidenceRecord(
            ...
            in QuicPacketFlushCadenceObservation observation)
        {
            ...
            bool rawPublished =
                owner.packetFlushCadenceObservations.Writer.TryWrite(
                    new PacketFlushCadenceEvidenceRecord(
...
internal readonly record struct AdaptiveBackpressureEvidenceRecord(
    string SchemaVersion,
    string ConnectionKey,
    QuicAdaptiveBackpressureObservation Observation);

internal readonly record struct PacketFlushCadenceEvidenceRecord(
    string SchemaVersion,
    string ConnectionKey,
...
internal enum AdaptiveRuntimeEvidenceMode
{
    Raw,
    BoundedAggregate,
}

internal static class AdaptiveRuntimeEvidenceModeExtensions
{
    internal static string ToContractValue(
        this AdaptiveRuntimeEvidenceMode mode)
        => mode switch
        {
            AdaptiveRuntimeEvidenceMode.Raw => "raw",
            AdaptiveRuntimeEvidenceMode.BoundedAggregate =>
                "bounded_aggregate",
