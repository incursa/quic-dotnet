...
# WI-QUIC-CRT-0010 - QUIC Server-Role Managed TLS Crypto Floor Work Item

## Summary

Extend the server role from ClientHello-only transcript staging into a permanent managed TLS crypto floor that accepts one supported ClientHello path, constructs a local ServerHello, derives Handshake packet-protection material, and exposes outbound ServerHello bytes through the existing library seam while leaving commit and wider handshake work unavailable.
...
- EncryptedExtensions, server certificate flight, server Finished, client-certificate authentication, trust-store policy, hostname validation, certificate-path validation, revocation, 0-RTT, 1-RTT, key update, endpoint-host wiring, harness changes, interop-runner handshake support, or any native TLS / QUIC fallback.
