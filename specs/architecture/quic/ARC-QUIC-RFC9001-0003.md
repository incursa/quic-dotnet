...
- Only the bounded local 1-to-0 repeated install plus lower-recovered retained phase-1 receive selection is implemented, and only after current-phase acknowledgment, three-PTO cooldown, and retained-old discard; peer-initiated repeated update cycles, next-key second-successor packet acceptance before local install, and broader repeated receive epoch selection remain unimplemented until the lifecycle ledger is widened and verified.
...
- Runtime AEAD-limit wiring is limited to first-update-or-discard; repeated epoch selection beyond the first successor, second-successor packet handling, and broader stateless-reset-only response behavior remain blocked until their lifecycle requirements and proof plans are traced.
...
- Repeated epoch selection, second-successor packet handling, and broader stateless-reset-only response behavior still need explicit lifecycle design and proof before implementation.
...
- Design and prove peer-initiated repeated update handling and next-key second-successor packet acceptance now that bounded local phase-2 install and lower-recovered retained phase-1 selection are in place.
- Keep repeated AEAD-limit-triggered local updates and broader stateless-reset-only response behavior blocked until their requirements are traced.
