...
- The next planned slice is runtime wiring for the helper-backed AEAD-limit policy decisions; repeated epoch selection beyond the first successor remains blocked until that control state is integrated with send/open paths.
...
- Wire helper-backed AEAD-limit decisions into the runtime 1-RTT send/open paths so local update, connection stop-use, and stateless-reset-only responses are applied without overusing exhausted keys.
