...
$Phases = @(
    (New-Phase -Id 'Phase 01' -Title 'Foundation — Wire Format and Packet/Frame Substrate' -Summary 'Start here. RFC 8999 invariants are already complete, so this phase begins with the remaining transport substrate and frame/header work that other phases depend on.' -Chunks @(
        (New-Chunk -ChunkId '8999-01-invariants' -Rfc '8999' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC8999.json' -SectionTokens @('S5P1') -Mode 'SKIP' -Reason 'Already implemented, tested, fuzzed, benchmarked, and closed out.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-21-long-header-general-and-initial' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S17','S17P1','S17P2') -Mode 'P2P3P4' -Reason 'Packet-header wire format and varint-related work already exists and carries stale VINT IDs.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-22-long-header-handshake-and-0rtt' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S17P2P1','S17P2P2','S17P2P3') -Mode 'P2P3P4' -Reason 'Version Negotiation and Initial packet header parsing already exist; Handshake/0-RTT packet semantics remain to be filled in.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-23-retry-version-short-header' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S17P2P4','S17P2P5','S17P2P5P1','S17P2P5P2','S17P2P5P3','S17P3','S17P3P1','S17P4') -Mode 'P2P3P4' -Reason 'Short-header and long-header envelope parsing exist, but Retry and remaining packet semantics are still incomplete.' -Confidence 'medium'),
        (New-Chunk -ChunkId '9000-24-frame-encodings-part-1' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S18','S18P1','S18P2') -Mode 'P3P4' -Reason 'Greenfield transport-parameter, PADDING, PING, and ACK frame work.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-25-frame-encodings-part-2' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S19P1','S19P2','S19P3','S19P3P1','S19P3P2','S19P4','S19P5') -Mode 'P2P3P4' -Reason 'This slice includes existing STREAM-frame parsing/tests and stale STRM IDs alongside greenfield frame encoding work.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-26-frame-encodings-part-3' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S19P6','S19P7','S19P8','S19P9','S19P10','S19P11') -Mode 'P2P3P4' -Reason 'This slice still overlaps existing STREAM-frame parsing/tests before moving into greenfield frame families.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-27-frame-encodings-part-4' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S19P12','S19P13','S19P14','S19P15','S19P16','S19P17','S19P18') -Mode 'P3P4' -Reason 'Greenfield frame-encoding work.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 02' -Title 'Connection Establishment and Crypto Bootstrap' -Summary 'After the wire substrate is in place, build connection establishment, CID policy, version negotiation behavior, TLS carriage, transport parameters, and address/path validation.' -Chunks @(
        (New-Chunk -ChunkId '9000-04-connection-ids-basics' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S5','S5P1','S5P1P1') -Mode 'P2P3P4' -Reason 'Existing packet-classification and Version Negotiation parser/tests already exist, so CID basics should reconcile first.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-05-connection-id-management' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S5P1P2','S5P2','S5P2P1','S5P2P2','S5P2P3') -Mode 'P2P3P4' -Reason 'Connection-ID management builds on existing packet parsing and CID-related logic.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-06-version-negotiation' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S6','S6P1','S6P2','S6P3') -Mode 'P3P4' -Reason 'Greenfield Version Negotiation behavior layered on top of existing header parsing.' -Confidence 'high'),
        (New-Chunk -ChunkId '9001-01-tls-core' -Rfc '9001' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9001.json' -SectionTokens @('S2','S3','S4','S5') -Mode 'P3P4' -Reason 'Greenfield QUIC TLS and packet-protection core.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-07-handshake-properties' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S7','S7P2','S7P3') -Mode 'P3P4' -Reason 'Greenfield cryptographic handshake and connection-ID authentication semantics.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-08-transport-params-and-crypto-buffers' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S7P4','S7P4P1','S7P4P2','S7P5') -Mode 'P3P4' -Reason 'Greenfield transport-parameter and CRYPTO buffering behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-09-address-validation-and-tokens' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S8','S8P1','S8P1P1','S8P1P2','S8P1P3','S8P1P4') -Mode 'P3P4' -Reason 'Greenfield address-validation token and amplification-limit behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-10-path-validation' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S8P2','S8P2P1','S8P2P2','S8P2P3','S8P2P4') -Mode 'P3P4' -Reason 'Greenfield path-challenge/response behavior.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 03' -Title 'ACK and Recovery Fundamentals' -Summary 'Once the handshake path exists, add acknowledgment generation, retransmission rules, RTT estimation, and loss detection so the connection can make forward progress reliably.' -Chunks @(
        (New-Chunk -ChunkId '9000-18-ack-generation' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S13','S13P1','S13P2','S13P2P1','S13P2P2','S13P2P3','S13P2P4','S13P2P5','S13P2P6','S13P2P7') -Mode 'P3P4' -Reason 'Greenfield ACK-generation behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-19-retransmission-and-frame-reliability' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S13P3') -Mode 'P3P4' -Reason 'Greenfield retransmission/frame-reliability work.' -Confidence 'high'),
        (New-Chunk -ChunkId '9002-01-transport-basics' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('S2','S3') -Mode 'P3P4' -Reason 'Greenfield ack-eliciting, packets-in-flight, and packet-number-space basics.' -Confidence 'high'),
        (New-Chunk -ChunkId '9002-02-rtt-estimation' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('S5','S5P1','S5P2','S5P3') -Mode 'P3P4' -Reason 'Greenfield RTT-estimation behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9002-03-loss-detection' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('S6','S6P1','S6P1P1','S6P1P2','S6P2','S6P2P1','S6P2P2','S6P2P2P1','S6P2P3','S6P2P4','S6P3','S6P4') -Mode 'P3P4' -Reason 'Greenfield loss detection and PTO behavior.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 04' -Title 'Streams and Flow Control' -Summary 'With basic recovery in place, implement stream abstractions, stream state machines, and flow control.' -Chunks @(
        (New-Chunk -ChunkId '9000-01-streams-core' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S2','S2P1','S2P2','S2P3','S2P4') -Mode 'P2P3P4' -Reason 'Existing stream parser, stream-ID parser, and STREAM-frame tests already exist and carry stale STRM IDs.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-02-stream-state' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S3','S3P1','S3P2','S3P3','S3P4','S3P5') -Mode 'P3P4' -Reason 'Greenfield stream state-machine work.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-03-flow-control' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S4','S4P1','S4P2','S4P4','S4P5','S4P6') -Mode 'P3P4' -Reason 'Greenfield stream and connection flow-control work.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 05' -Title 'Migration and Path Management' -Summary 'Once streams, ACK, and validation exist, implement migration and path-management behavior.' -Chunks @(
        (New-Chunk -ChunkId '9000-11-migration-core' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S9','S9P1','S9P2','S9P3','S9P3P1','S9P3P2','S9P3P3') -Mode 'P3P4' -Reason 'Greenfield migration-core behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-12-migration-followup' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S9P4','S9P5','S9P6','S9P6P1','S9P6P2','S9P6P3','S9P7') -Mode 'P3P4' -Reason 'Greenfield migration follow-up and preferred-address behavior.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 06' -Title 'Lifecycle, Close, and Error Handling' -Summary 'Add idle timeout, connection close, stateless reset, and error signaling after the main path is alive.' -Chunks @(
        (New-Chunk -ChunkId '9000-13-idle-and-close' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S10','S10P1','S10P1P1','S10P1P2','S10P2','S10P2P1','S10P2P2','S10P2P3') -Mode 'P3P4' -Reason 'Greenfield idle-timeout and connection-close behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-14-stateless-reset' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S10P3','S10P3P1','S10P3P2','S10P3P3') -Mode 'P3P4' -Reason 'Greenfield stateless-reset behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-15-error-handling' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S11','S11P1','S11P2') -Mode 'P3P4' -Reason 'Greenfield transport/application error-handling behavior.' -Confidence 'high')
    )),
    (New-Phase -Id 'Phase 07' -Title 'Congestion Control and Recovery State' -Summary 'After loss detection exists, add congestion control and then review appendix-driven recovery-state implementation work.' -Chunks @(
        (New-Chunk -ChunkId '9002-04-congestion-control' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('S7','S7P1','S7P2','S7P3P1','S7P3P2','S7P3P3','S7P4','S7P5','S7P6','S7P6P1','S7P6P2','S7P7','S7P8') -Mode 'P3P4' -Reason 'Greenfield congestion-control, ECN, and persistent-congestion behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9002-05-appendix-a-recovery-state' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('SAP1','SAP1P1','SAP2','SAP4','SAP5','SAP6','SAP7','SAP8','SAP9','SAP10','SAP11') -Mode 'REVIEW' -Reason 'Inventory marked this appendix slice human_review_first because the retained SAP11/BP9 overlap pair needs manual judgment before automation.' -Confidence 'medium')
    )),
    (New-Phase -Id 'Phase 08' -Title 'Late, Policy-Heavy, and Appendix Work' -Summary 'Finish the smaller late-policy slices and then review the remaining appendix B material.' -Chunks @(
        (New-Chunk -ChunkId '9001-02-security-and-registry' -Rfc '9001' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9001.json' -SectionTokens @('S6','S7','S8','S9','S10') -Mode 'P3P4' -Reason 'Greenfield key-update, security-consideration, and registry behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9001-03-appendix-b-aead-limits' -Rfc '9001' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9001.json' -SectionTokens @('SB','SBP1P1','SBP1P2','SBP2') -Mode 'P3P4' -Reason 'Greenfield appendix B AEAD limit behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-20-datagram-and-mtu' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S13P4','S13P4P1','S13P4P2','S13P4P2P1','S13P4P2P2','S14','S14P1','S14P2','S14P2P1','S14P3','S14P4') -Mode 'P3P4' -Reason 'Greenfield datagram-size and PMTU behavior.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-28-errors-registry-and-security' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S19P19','S19P20','S19P21','S20P1','S20P2','S21P1P1P1','S21P2','S21P3','S21P4','S21P5','S21P5P3','S21P5P6','S21P6','S21P7','S21P9','S21P10','S21P11','S21P12') -Mode 'P3P4' -Reason 'Greenfield error-code, security, and late-policy material.' -Confidence 'high'),
        (New-Chunk -ChunkId '9000-29-iana-and-late-sections' -Rfc '9000' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9000.json' -SectionTokens @('S22P1P1','S22P1P2','S22P1P3','S22P1P4','S22P2','S22P3','S22P4','S22P5') -Mode 'P3P4' -Reason 'Greenfield IANA and late-section work.' -Confidence 'high'),
        (New-Chunk -ChunkId '9002-06-appendix-b-constants-and-examples' -Rfc '9002' -SpecFile './specs/requirements/quic/SPEC-QUIC-RFC9002.json' -SectionTokens @('SBP1','SBP2','SBP3','SBP4','SBP5','SBP6','SBP7','SBP8','SBP9') -Mode 'REVIEW' -Reason 'Inventory marked this appendix slice human_review_first because the retained SAP11/BP9 overlap pair needs manual judgment before automation.' -Confidence 'medium')
    ))
)
