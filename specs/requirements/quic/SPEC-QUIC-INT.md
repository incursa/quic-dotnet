...
The interop harness MUST define a child-process-only `multiconnect` testcase in which the ordered space-separated client `REQUESTS` URLs are interpreted as a sequential dispatch plan, one managed connection is opened per URL on the existing endpoint-host active-phase path, and the client sends exactly one HTTP/0.9 GET and downloads exactly one mounted response per connection. For server-role runs with an ordered `REQUESTS` list, the server MUST accept one connection per URL, accept exactly one request stream, and serve exactly one mounted response per connection. For server-role runs where the runner provides an empty `REQUESTS` list, the server MUST treat the testcase as an open sequential accept plan, derive each mounted response path from the observed HTTP/0.9 request target, serve after a complete request line without waiting for request EOF, and after each final response wait for peer close or a bounded timeout without initiating application close solely to end the connection or blocking later sequential accepts and handshakes. Exit code `0` is considered honest only after byte delivery plus EOF have completed for every ordered client response, and a response that stalls before byte delivery plus EOF complete MUST fail the child process with exit code `1` rather than hanging behind the external runner timeout. This slice must not claim parallel connection handling, generalized multipath routing, migration or rebinding support, retry enablement, `0-RTT`, key update, or any public API widening.
...
- Server-role open-plan support exists because the external runner supplies an empty `REQUESTS` list to the server side while the peer client still opens sequential connections and sends concrete HTTP/0.9 targets.
