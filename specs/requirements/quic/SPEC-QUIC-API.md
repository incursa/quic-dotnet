...
- Combined `Abort(Both, ...)` and broader stream-management parity remain deferred.
...
The library MUST classify terminal and shutdown outcomes through QuicError and QuicException, surface application and transport error codes on the exception, and support per-stream abort direction through QuicAbortDirection so consumers can distinguish read, write, and both-side termination without referencing internal protocol state.
...
- Combined `Abort(Both, ...)` remains deferred and MUST be rejected explicitly instead of being silently ignored.
...
- The write/completion requirement itself remains limited to `QuicAbortDirection.Write`; `Abort(Read, ...)` is specified separately in `REQ-QUIC-API-0006`, and `Abort(Both, ...)` remains explicitly unsupported until a combined read/write abort slice exists.
