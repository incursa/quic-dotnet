...
Define the intended public API surface for Incursa.Quic using the public surface of System.Net.Quic as the baseline for the initial consumer cut, the first honest server-entry slice, the first honest client-entry slice, the first honest stream-entry slice, the narrow supported write/completion slice, the narrow reset/stop-sending abort slice, the first honest stream-capacity callback slice, the narrow supported close-driven capacity-release follow-on, and the shared `IsSupported` capability marker slice.
