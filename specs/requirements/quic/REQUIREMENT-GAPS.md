...
- `9001-05-handshake-orchestration` is still incomplete overall. The library now has a transport-facing TLS bridge state under `REQ-QUIC-CRT-0103`, a narrow Handshake flow coordinator under `REQ-QUIC-CRT-0106`, a role-aware transcript-progress owner under `REQ-QUIC-CRT-0107`, and a new client-only managed TLS 1.3 crypto slice under `REQ-QUIC-CRT-0108` and `REQ-QUIC-CRT-0109`. That slice is intentionally narrow: it is limited to `TLS_AES_128_GCM_SHA256` with `secp256r1`, derives handshake secrets from the local ephemeral share, peer `ServerHello` key share, and transcript hash, verifies peer `Finished` cryptographically, and gates peer transport-parameter commit on `PeerFinishedVerified`. Remaining blockers are certificate validation, `CertificateVerify` signature verification, identity-authentication semantics, 0-RTT, 1-RTT, key update, and any server-role handshake crypto or commit path.
