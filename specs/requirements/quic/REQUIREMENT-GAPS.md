...
- `9001-05-handshake-orchestration` is still incomplete overall. The library now has a narrow Handshake flow coordinator under `REQ-QUIC-CRT-0106` plus a role-aware transcript-progress owner under `REQ-QUIC-CRT-0107` that can open and protect Handshake packets, move CRYPTO bytes through the bridge driver, consume ordered peer handshake messages, and stage peer transport parameters from the correct role-specific message. Certificate validation, signature verification, peer-authentication policy, handshake confirmation, 1-RTT rollout, and key-update semantics remain blocked.
