...
- `9001-05-handshake-orchestration` is still incomplete overall. The library now has a narrow Handshake flow coordinator under `REQ-QUIC-CRT-0106` that can open and protect Handshake packets and move CRYPTO bytes through the bridge driver, but certificate validation, transcript ownership, peer-authentication policy, 1-RTT rollout, and key-update semantics remain blocked.
