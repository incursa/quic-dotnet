...
## REQ-QUIC-CRT-0127 Own opaque resumption-ticket state and keep early data gated closed
In the client role, the library MUST own opaque resumption-ticket state on the managed connection path, capture and retain one server-issued ticket through the later connection replay boundary, and keep the early-data gate explicitly closed until that owned state is present and replayable. This slice MUST NOT derive PSKs, send or accept 0-RTT payloads, add anti-replay guarantees, widen the public API, or alter transfer or retry contracts.
