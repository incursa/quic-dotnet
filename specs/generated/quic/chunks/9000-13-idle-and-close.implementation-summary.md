...
- Added requirement-home tests for the close-path helper slice so the lifecycle helper now proves closing-state entry, draining-state no-send behavior, and peer-close draining without inventing a packet sender.

## Tests Run and Results
- `dotnet test tests/Incursa.Quic.Tests/Incursa.Quic.Tests.csproj --no-restore --filter "FullyQualifiedName~REQ_QUIC_RFC9000_0572|FullyQualifiedName~REQ_QUIC_RFC9000_S10P2P2_0001|FullyQualifiedName~REQ_QUIC_RFC9000_S10P2P2_0003|FullyQualifiedName~REQ_QUIC_RFC9000_S10P2P2_0004"`
- Result: `7 passed, 0 failed, 0 skipped`
- `dotnet test tests/Incursa.Quic.Tests/Incursa.Quic.Tests.csproj --no-restore --filter "FullyQualifiedName~REQ_QUIC_RFC9000_S10P2P2_0005"`
...
- Idle timeout expiration is now detectable, and peer-close draining is now proven, but silent close and immediate close still require a connection controller to act on that state.
