...
Verify the endpoint-host shell, the managed client/listener Initial bootstrap seam, handshake-only harness dispatch into that seam, the connected-UDP bridge into the library-owned runtime endpoint, the outbound datagram handoff, the honesty of route-miss handling, and the real child-process completion / exit-0 proof for the handshake testcase.
