...
# VER-QUIC-CRT-0045 - QUIC server-role secp256r1 HelloRetryRequest verification

## Scope

Verify that the managed server-role handshake path emits exactly one HelloRetryRequest when the first ClientHello advertises secp256r1 in supported_groups but omits a usable secp256r1 key_share, keeps ServerHello plus Handshake key publication unavailable until the retried ClientHello is accepted, and then rejoins the landed REQ-QUIC-CRT-0112 ServerHello floor without widening the cryptographic subset.
