...
# VER-QUIC-CRT-0015 - QUIC Server-Role Inbound Client Finished Proof Verification

## Scope

This slice verifies only the permanent server-role continuation from peer ClientHello and local ServerHello through local EncryptedExtensions, local Certificate, local CertificateVerify, local Finished, and inbound client Finished proof, plus Handshake packet-protection material availability. It does not prove server-role commit, client-certificate authentication, trust-store policy, hostname validation, certificate-path validation, revocation, 0-RTT, 1-RTT, key update, endpoint-host wiring, or interop-runner handshake support.
...
Focused requirement-home tests and inspection evidence for the narrow server-role inbound client Finished proof behind the existing transcript-progress and bridge-driver seam.
