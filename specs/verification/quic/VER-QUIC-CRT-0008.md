...
This slice verifies the permanent client-only managed TLS 1.3 key-schedule seam, the explicit bridge-visible `PeerFinishedVerified` state, and the commit gate that depends on cryptographic peer Finished verification. It does not prove certificate validation, `CertificateVerify` signature verification, 0-RTT, 1-RTT, key update, or server-role handshake crypto.
...
- Inspect the bridge state and runtime integration to confirm that handshake keys are not published before the required cryptographic inputs exist and that `PeerTransportParametersCommitted` cannot occur before `PeerFinishedVerified`.

## Expected Result

The bridge exposes handshake-key availability only after the managed key-schedule inputs exist, unsupported or malformed key-share and cipher-suite input fails deterministically, transcript-hash-driven derivation is repeatable, peer Finished verification succeeds on the supported client path and fails deterministically on mismatch, peer transport-parameter commit is gated by `PeerFinishedVerified`, fatal crypto state blocks later commit progression, and server-role commit remains unavailable. This does not imply certificate validation, identity validation, or full handshake support.
