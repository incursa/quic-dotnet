...
Last verified: 2026-05-19 local QUIC-protocol audit, local all-upstream client-role QUIC testcase refresh, and local clean-runner `connectionmigration` server-role proof. HTTP/3 is intentionally out of scope. SpecTrace core validation passed for 568 JSON artifacts, generated QUIC requirement coverage reports 1,771/1,771 RFC 8999/9000/9001/9002 requirements as `trace_clean`, Release build passed with 0 warnings and 0 errors, the full test suite passed 5,217/5,217 before the focused ACK-threshold topoff, and the current focused ACK/stream/key-update/interoperability tests passed. The remaining QUIC-only open/advisory work is evidence-scoped, not an RFC requirement coverage gap: hosted `REQ-QUIC-INT-0025` connectionmigration corroboration remains advisory pending a fresh hosted run, `SIM-QUIC-LOSS-0001` deterministic simulator loss evidence remains open, xquic remains the only failed local client-role all-upstream `transfer`/`keyupdate`/`chacha20` peer cluster, and broader non-HTTP/3 server-role all-upstream testcase coverage remains advisory outside the currently green handshake plus local connectionmigration proof lanes.
...
## 2026-05-19 INT Connectionmigration Local Proof

`REQ-QUIC-INT-0025` formalized the connectionmigration source-address blocker as an advisory hosted proof lane. The local clean-runner refresh now shows the source-address fix working: `pwsh -NoProfile -File scripts\interop\Invoke-QuicInteropRunner.ps1 -RunnerRoot C:\src\quic-interop\quic-interop-runner -LocalRole server -ImplementationSlot nginx -PeerImplementationSlots neqo-peer -TestCases connectionmigration -ArtifactsRoot .artifacts\interop-runner\connectionmigration-shim-proof -RunnerTimeoutSeconds 240` completed with exit code 0 under `.artifacts/interop-runner/connectionmigration-shim-proof/20260519-163904754-server-nginx`, and the pcap's first server packets are sourced from the original server address before later preferred-address path use. The preferred migration address remains configured with `preferred_lft 0` so it is reachable for migration without becoming the kernel-preferred source for the original-path first flight. The hosted lane remains advisory until the manual hosted proof is refreshed with the same repo-owned shim.
...
proof run using the local `nginx` replacement slot against `neqo`; none of them are blocked on
