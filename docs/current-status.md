...
`REQ-QUIC-INT-0022` now splits the hosted `connectionmigration` evidence into a proof profile that exercises the server role against `lsquic` using the `neqo` slot, plus a companion `connectionmigration-server-proof-blocked` profile that preserves `picoquic`, `quiche`, `ngtcp2`, `quic-go`, `msquic`, and `aioquic` comparisons while also keeping `lsquic` visible for side-by-side proof. The latest hosted `ngtcp2` attempt hit image-signature validation issues, so the proof lane is being retargeted back to the `neqo`/`lsquic` pairing rather than being claimed green too early. The blocked comparison lane stays advisory and remains separated from the major-peer matrix.
...
proof run using the `neqo` slot against `lsquic`; none of them are blocked on
