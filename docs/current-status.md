...
comparisons. The latest hosted `picoquic` attempt failed on image-signature
validation, so the proof lane is now being re-aimed at the `lsquic` slot
against `ngtcp2` instead of claiming a green result too early. The blocked
...
hosted proof run using the `lsquic` slot against `ngtcp2`; none of them are
