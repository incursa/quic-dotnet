...
Last verified: 2026-06-18 bounded QUIC interop-runner `http3` adapter build/unit/dry-run proof plus live local quic-go/quic-go `http3` runner proof, RFC9368 compatible-version runtime-selection closeout, RFC9221 QUIC DATAGRAM transport-floor closure, RFC9000 repeated MAX_STREAM_DATA/MAX_DATA runtime proof, RFC9220 WebSocket application-close, protocol-error close, manual server-ping helper, handler-exception close, opt-in keepalive scheduler proof, TCP forwarder proof, TCP forwarder bounded-buffer proof, client-side WebSocket tunnel proof, WebSocket tunnel metadata proof, keepalive payload proof, handler-exception close-policy proof, and dynamic handler-exception close-policy proof, xquic captured-packet and credit-burst proof, non-promoting xquic runner diagnostic proof, the June 18 local xquic `transfer`/`keyupdate`/`chacha20` refresh, 2026-05-22 local simulator promotion review, regenerated RFC requirement coverage triage, advisory peer-characterization report closeout, the 2026-05-20 hosted `connectionmigration-server-proof` run `26174181713`, the May 22 focused xquic transfer rerun, and the May 22 `rebind-addr` server-trace-shim rerun. HTTP/3 is now present as a bounded interop-runner adapter over the standalone HTTP/3/QPACK modules; broader HTTP/3 production hosting, dynamic-QPACK-dependent support, server push, trailers, CONNECT, HTTP Datagrams, and MASQUE remain intentionally out of scope. Core SpecTrace JSON validation passes for 906 artifacts, and generated QUIC requirement coverage reports 2,674/2,674 requirements as `trace_clean`. The latest focused RFC9220 WebSocket filter passed 48/48, the latest focused RFC9221/API filter passed 106/106, the latest focused RFC9368 filter passed 20/20, the targeted v2 interop sanity checks passed 2/2, the full test project passed 5,530/5,530, the RFC9221 DATAGRAM frame benchmark dry run executed all 4 parse/format benchmarks with `--inProcess`, the focused RFC9000 MAX credit publication filter passed 7/7, the focused xquic/runner diagnostic proof set passed 30/30, the focused simulator-accounting verification passed 9/9, and the June 18 Release/full-suite floor passed 7,483/7,488 with 5 skipped. The remaining QUIC-only open/advisory work is evidence-scoped, not an RFC requirement coverage gap: RFC9221 now has bounded public send/receive DATAGRAM support with transport-parameter negotiation, DATAGRAM frame codec support, 1-RTT protection, congestion/drop, non-retransmission, flow-control exclusion, and benchmark evidence; HTTP Datagrams, CONNECT-UDP, MASQUE, application-facing DATAGRAM ACK/loss callbacks, and 0-RTT DATAGRAM transmission remain separate future work. `REQ-QUIC-INT-0030` owns the runner-facing `http3` Docker endpoint cell; the 2026-06-16 live local quic-go/quic-go `http3` runner succeeded under `.artifacts/interop-runner/http3-local-live-current/20260616-184828293-both-quic-go`, with `runner-report.json` recording `result: succeeded`. `REQ-QUIC-INT-0025` connectionmigration hosted corroboration is green for the bounded `connectionmigration` profile, `VER-QUIC-INT-0020` now accepts the preserved baseline and deterministic-loss simulator evidence as bounded proof for the mapped requirements, `VER-QUIC-INT-0016` closes the advisory peer-characterization reporting surface as non-promoting evidence, `REQ-QUIC-INT-0029` now owns the remaining xquic client-role all-upstream `transfer`/`keyupdate`/`chacha20` residual, proves the managed client emitted a decryptable ACK through xquic response packet 17 plus repeated increasing MAX_DATA/MAX_STREAM_DATA updates through client packet 28, and adds a non-promoting runner diagnostic for the preserved timeout shape plus xquic's server-log packet-processing ceiling through packets 0..3; the rebind cells now have June 11, 2026 direct quic-go/chrome runner proof and are supported/executed inventory cells rather than prerequisite-blocked cells. RFC9000 reference reconciliation should use the derived crosswalk and retired-ID ledger in `specs/generated/quic` before changing live requirement references.

`REQ-QUIC-INT-0031` now owns the advisory external HTTP/3 peer harness shape: Docker Compose targets, Windows/POSIX runners, JSONL results, peer-tool acquisition manifests, and Markdown report generation for Incursa, curl, aioquic, quiche/h3i, and ngtcp2/nghttp3 target cells. This is a characterization surface with explicit `skip` rows for unwired peer/scenario cells, not proof of broad HTTP/3 peer interoperability.

Local evidence for this slice passed `dotnet build Incursa.Quic.slnx -c Release --no-restore`, passed the focused `REQ_QUIC_INT_0031` requirement-home test filter 2/2, passed POSIX script syntax checking with `bash -n`, and produced a plan-only report under `.artifacts/http3-external-plan/20260525T212334Z/report.md` with 80 explicit `skip` rows.

The 2026-06-19 refresh pinned the repo-local aioquic peer image to `aioquic==1.3.0`, normalized comma-separated PowerShell `-Targets` and `-Scenarios`, and wrote `peer-tool-manifest.json` for each PowerShell run. The plan-only normalized matrix under `.artifacts/http3-external/20260619T055646Z` preserved 24 explicit rows for the focused target/scenario set, and the live focused Docker subset under `.artifacts/http3-external/20260619T055801Z` passed 6/6 static GET-style rows for `incursa-client__incursa-server`, `aioquic-client__incursa-server`, and `incursa-client__aioquic-server` across `get-small` and `not-found`. The follow-up external-client run under `.artifacts/http3-external/20260619T125600Z` passed 12/12 rows for `curl__incursa-server`, `quiche-client__incursa-server`, and `ngtcp2-client__incursa-server` across `get-small`, `not-found`, `get-large`, and `many-headers`. The scenario-breadth run under `.artifacts/http3-external/20260619T130112Z` passed 7/8 rows: curl, quiche client, and ngtcp2/nghttp3 client passed `get-empty` and `split-data`, and `incursa-client__quiche-server` passed `get-empty` while `split-data` stayed explicitly skipped as unwired for that peer-server row. The peer-server follow-up under `.artifacts/http3-external/20260619T124926Z` passed `incursa-client__quiche-server` for `get-small` and `not-found` while `incursa-client__ngtcp2-server` still exposed startup-sensitive handshake failures; after the harness began creating ngtcp2 qlog and sslkeylog directories before `wsslserver` starts, `.artifacts/http3-external/20260619T130437Z` passed `incursa-client__ngtcp2-server` for `get-small` and `get-empty` and kept `not-found` and `get-large` as executable Incursa-client handshake-timeout failures. `.artifacts/http3-external/20260619T125037Z` separately passed `incursa-client__quiche-server` for `get-large`. The RFC 9287 fixed-bit receive fix was then corroborated by `.artifacts/http3-external/20260619T184808Z`, which passed `incursa-client__ngtcp2-server` for both `not-found` and `get-large`. The manifest now records resolved image IDs/digests, roles, supported scenarios, command templates, and known limitations for curl, aioquic, quiche, and ngtcp2/nghttp3. This remains advisory external-peer characterization; it does not promote broad HTTP/3 or dynamic QPACK support.

The 2026-06-19 ECN receive-metadata feasibility slice added a no-op managed receive seam through `QuicSocketEcnControl.TryGetReceivedEcnCounts` and threaded it through the endpoint/listener receive loops. The seam preserves current behavior because `Socket.ReceiveMessageFromAsync` and `SocketReceiveMessageFromResult` still do not expose `IP_TOS` or `IPV6_TCLASS` ancillary ECN bits; production receive-side ECN promotion remains native/control-message work.

`REQ-QUIC-INT-0032` now owns the advisory h3spec HTTP/3 server conformance pipeline: local server lifecycle scripting, repo-local h3spec acquisition under `.artifacts/tools`, h3spec stdout/stderr capture, parsed JSON results, Markdown failure triage, RFC 9114/RFC 9204 failure mapping, and an opt-in CI workflow. Local evidence passed the focused `REQ_QUIC_INT_0032` requirement-home test filter 3/3, produced plan-only h3spec artifacts under `.artifacts/http3-h3spec-plan/20260525T213648Z`, proved parser mapping with sample HTTP/3 and QPACK failures under `.artifacts/http3-h3spec-parser-smoke`, acquired upstream h3spec `v0.1.13` under `.artifacts/tools/h3spec-v0.1.13`, and ran live local acquired h3spec under `.artifacts/http3-h3spec/20260619T0037Z-acquired-live`. The live run executed 49 cases and reported 41 failures, including 15 RFC 9114/RFC 9204 failures mapped to `http3-adapter-boundary` and `qpack-stream-state-boundary`; this remains conformance triage and does not change HTTP/3/QPACK support claims until protocol-owned requirements and tests close those failures.

`VER-QUIC-RFC9220-0018` now records repeatable RFC9220 WebSocket local-loopback, local cross-process, and local external aioquic peer proof. `pwsh -NoProfile -File scripts/proofs/Invoke-Rfc9220WebSocketExternalAioquicProof.ps1 -RunId 20260619T-rfc9220-aioquic-001` passed with the Incursa sample server process and an `aioquic==1.3.0` HTTP/3 client container, writing `.artifacts/http3-websocket-external-aioquic/20260619T-rfc9220-aioquic-001`. The run covers Extended CONNECT setup, accepted metadata, server ping/aioquic pong, aioquic ping/server pong, text echo, 6000-byte binary echo, aioquic close, and server close echo. Evidence class is `local-external-aioquic-peer`; isolated-host proof, browser proof, publishable evidence, and production proxy policy remain out of scope. aioquic image ID `sha256:b022b70e9f351326bf2951c5a56c5a00808b9f0b226a363d8b1b27014352d975`; manifest SHA256 `0FF94FBC204441D15CC85B644712DAE371DF80E943AE0A6DC156D344DF56BB2F`; report SHA256 `6BBD7177BDB2FCC6C390268187FCC365C73A2112A589483228457C0A7B0B15F5`; client-result SHA256 `2046050B070549787E236CDA5B175E6E737E6224615CA02962C2D68C4E3A9273`.

`REQ-QUIC-RFC9114-S4-0001` now owns the protocol-side low-level malformed HTTP/3 sequence test surface. The custom test client covers DATA before HEADERS, duplicate SETTINGS, SETTINGS on request streams, HEADERS on control streams, invalid frame lengths, unknown and reserved frame handling, pseudo-header ordering, uppercase fields, invalid Content-Length, server-initiated bidirectional stream rejection, and duplicate control streams. Local evidence passed `Http3LowLevelProtocolTests` 12/12 and the focused RFC9114 requirement-home filter 3/3; this is layer-local RFC 9114 classification proof, not live h3i wire proof.
...
`REQ-QUIC-INT-0022` now splits the hosted `connectionmigration` evidence into a proof profile that exercises the server role by replacing the distinct local `nginx` slot while keeping `neqo-peer` as the reported peer alias, normalizing the actual runner client slot to `neqo`, and using the `ghcr.io/mozilla/neqo-qns:latest` peer image. The companion `connectionmigration-server-proof-blocked` profile preserves `ngtcp2`, `lsquic`, `quiche`, `quic-go`, `msquic`, and `aioquic` comparisons. Hosted run `25864019585` cleared the alias lookup failure but exposed a replacement-slot collision: the runner used `-s neqo -c neqo -r neqo=incursa-quic-interop-harness:local`, so the later `single path in use` failure is not accepted as a real neqo-peer migration verdict. Hosted run `25866825495` confirmed the distinct-slot runner shape (`-s nginx -c neqo -r nginx=incursa-quic-interop-harness:local`) but failed before transfer or migration with server-side `TLS alert 50`. After the ClientHello compatibility topoff, hosted run `25867235353` still failed at the same pre-transfer TLS alert (`interop harness: role=server, testcase=connectionmigration failed: TLS alert 50`), with the client receiving transport error 10. Hosted run `25868496912` kept the correct runner invocation and moved past the TLS alert, but exposed the earlier source-address failure: the first server flight was sourced from preferred migration address `fd00:cafe:cafe:100::110` while the client connected to original server address `fd00:cafe:cafe:100::100`. The runtime and harness setup then gained local proof for that corrected path. Local clean-runner run `.artifacts/interop-runner/connectionmigration-shim-proof/20260519-163904754-server-nginx` passed `connectionmigration` against `neqo`: the first server packets came from `fd00:cafe:cafe:100::100`, the analyzer observed both original and preferred server paths, and `runner-report.json` records `result: succeeded`. Hosted run `26174181713` at commit `9ee3266e` then passed the `connectionmigration-server-proof` profile with `runner-report.json` recording `connectionmigration` as `succeeded`, a completed 2 MiB response, original and preferred server paths observed, qlog-backed PATH_CHALLENGE/PATH_RESPONSE evidence, and active-migration DCID change to `20:21:22:23:24:25:26:27`. The repo-owned runner shim carries the keylog-injection, preferred-address analyzer compatibility, qlog fallback, and malformed analyzer-packet tolerance needed to reproduce this without a dirty upstream runner checkout. Hosted run `25882671754` remains preserved as blocked comparison evidence across `ngtcp2`, `quiche`, `lsquic`, `quic-go`, `msquic`, and `aioquic`; those peers failed with mixed issues including quic-go zero-length-DCID CID-retirement rejection, msquic packet-layer `KeyError`, and ngtcp2 transfer-size mismatch. The blocked comparison lane stays advisory and remains separated from the major-peer matrix.
...
## 2026-05-20 INT Connectionmigration Hosted Proof

`REQ-QUIC-INT-0025` now closes the connectionmigration source-address blocker for the bounded `connectionmigration` hosted proof lane. The local clean-runner refresh first showed the source-address fix working: `pwsh -NoProfile -File scripts\interop\Invoke-QuicInteropRunner.ps1 -RunnerRoot C:\src\quic-interop\quic-interop-runner -LocalRole server -ImplementationSlot nginx -PeerImplementationSlots neqo-peer -TestCases connectionmigration -ArtifactsRoot .artifacts\interop-runner\connectionmigration-shim-proof -RunnerTimeoutSeconds 240` completed with exit code 0 under `.artifacts/interop-runner/connectionmigration-shim-proof/20260519-163904754-server-nginx`, and the pcap's first server packets are sourced from the original server address before later preferred-address path use. Hosted run `26174181713` then corroborated that path under `.artifacts/gh-run-26174181713/interop-runner-server-connectionmigration-neqo-26174181713/20260520-155738932-server-nginx`: the runner report records `connectionmigration` as `succeeded`, output records the completed 2 MiB response, both original and preferred server paths were observed, qlog confirmed server PATH_CHALLENGE/client PATH_RESPONSE evidence, and the active-migration DCID changed to `20:21:22:23:24:25:26:27`. The preferred migration address remains configured with `preferred_lft 0` so it is reachable for migration without becoming the kernel-preferred source for the original-path first flight. This does not promote `rebind-port`, `rebind-addr`, QUIC v2/RFC 9369, or HTTP/3.
...
evidence aligned across transfer, keyupdate, and chacha20. The June 18 refresh
under
`.artifacts/interop-runner/debug-client-xquic-refresh/20260618-085552662-client-chrome`
reran `transfer`, `keyupdate`, and `chacha20` together; `runner-report.json`
kept all three cells failed, each client timed out waiting for response-stream
FIN after 15,355 bytes, and the xquic server logs reached
`xqc_process_read_streams|stream_read_notify|flag:66`, scheduled response
bytes through `stream_offset:16536`, and then reported
`xqc_send_packet_pacer_allows|pacing blocked`. This is a non-promoting external
xquic liveness refresh rather than a local ACK/protection proof. The helper now
also has a shared-ceiling fallback for `keyupdate` and `chacha20` when the
server log does not expose the deeper packet markers.
...
The permanent-registration policy slice is now closed for
...
`RFC9000-S6-3-P2-R01` is now `trace_clean` under the baseline RFC 9000
...
`RFC9000-S13-2-2-P5-S1-R01` is now `trace_clean` under the existing
...
`RFC9000-S13-2-7-P1-S3-R01` and `RFC9000-S13-P2-S2-R01` are now
...
2026-05-06 closes `RFC9000-S8-1-4-P3-S2-R01`,
`RFC9000-S8-1-4-P5-S3-R01`, and
...
2026-05-06 closes `RFC9000-S9-6-1-P4-S2-R01` under
...
`REQ-QUIC-RFC9000-S5P2-0009`, and `RFC9000-S5-2-P6-S1-R01`
...
2026-05-06 closes `RFC9000-S7-4-1-P4-S2-R01` under
...
on 2026-05-06 closes `RFC9000-S7-4-1-P6-R01` through
...
remaining non-clean requirements: `RFC9000-S7-4-1-P4-S2-R01` and
`REQ-QUIC-RFC9000-S7P4P1-0010` through
...
`RFC9000-S7-4-1-P4-S2-R01` and `RFC9000-S7-4-1-P6-R01` through
...
`RFC9000-S17-2-5-2-P1-R01` through
...
`RFC9000-S17-2-5-3-P4-S6-R01` remains optional server abort behavior and
...
`RFC9000-S5-2-2-P5-S1-R01` and
`RFC9000-S5-2-2-P6-S1-R01` under
...
`RFC9000-S19-13-P2-R01`. The S19P13 proof now covers type `0x15`,
...
2. Regenerated coverage triage marks `REQ-QUIC-RFC9000-S7P3-0001` through
   ...
   the next local lane.
3. Regenerated coverage triage marks `RFC9000-S17-2-5-2-P1-R01`
   through `REQ-QUIC-RFC9000-S17P2P5P2-0009` and
   `REQ-QUIC-RFC9000-S17P2P5P3-0001` through
   `REQ-QUIC-RFC9000-S17P2P5P3-0007` as `trace_clean`.
4. `RFC9000-S17-2-5-3-P4-S6-R01` remains optional, unclaimed server abort
...
6. Regenerated coverage triage marks `RFC9000-S19-13-P1-R01` through
