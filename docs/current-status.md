...
proof profile against `neqo` with `xquic` plus a companion
`connectionmigration-server-proof-blocked` profile that preserves `picoquic`,
`ngtcp2`, `neqo`, `lsquic`, `quiche`, `quic-go`, `msquic`, and `aioquic`
comparisons. The helper inventory already classifies `connectionmigration` as
supported/executed, but the latest hosted `neqo`/`quinn` attempt stalled during
handshake, so the proof lane is being re-aimed at the public `neqo`/`xquic`
candidate instead of claiming a green result too early. The blocked comparison
