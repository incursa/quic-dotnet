...
`lsquic`, `quiche`, `quic-go`, `msquic`, `neqo`, and `aioquic`
comparisons. The helper inventory already classifies `connectionmigration` as
supported/executed, but the latest hosted `lsquic` attempt failed image-signature
validation, so the proof lane is being re-aimed at `picoquic` instead of
