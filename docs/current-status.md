...
`REQ-QUIC-INT-0032` now owns the advisory h3spec HTTP/3 server conformance pipeline: local server lifecycle scripting, h3spec stdout/stderr capture, parsed JSON results, Markdown failure triage, RFC 9114/RFC 9204 failure mapping, and an opt-in CI workflow. Local evidence passed the focused `REQ_QUIC_INT_0032` requirement-home test filter 3/3, produced plan-only h3spec artifacts under `.artifacts/http3-h3spec-plan/20260525T213648Z`, and proved parser mapping with sample HTTP/3 and QPACK failures under `.artifacts/http3-h3spec-parser-smoke`. This remains conformance triage and does not change HTTP/3/QPACK support claims.
