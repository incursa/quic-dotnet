...
| `REQ-QUIC-INT-0025` connectionmigration source-address proof | Advisory | The local clean-runner proof now passes with the first server flight on the original address and later preferred-address path use; hosted corroboration has not yet been refreshed. | Dispatch the hosted proof lane with the repo-owned analyzer shim before promoting hosted readiness. |
