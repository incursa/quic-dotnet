...
- 2026-07-08: client response sequence validation now has an internal owned-header path so decoded QPACK response headers can be retained without a second defensive `ToArray()` copy, matching the existing request validator pattern while preserving the public copy-on-receive behavior. Focused HTTP/3 header/client/RFC 9114 tests passed. Existing allocation benchmarks do not directly cover this client response validator path, so treat this as targeted allocation cleanup backed by tests rather than measured benchmark movement.
