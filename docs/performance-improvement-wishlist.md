...
### Accepted 2026-07-16: decouple stream-action registration from transport mutation

The 100x1 KiB multiplex ladder isolated a high-concurrency collapse after c16:
7.28 MiB/s at c1, 17.65 at c4, 24.44 at c16, 20.97 at c32, 13.61 at c64,
and 8.47 at c128. Target CPU fell while memory and handle counts rose, so the
first candidate investigated queue ownership rather than compute saturation.
The original `pendingStreamActionRequestsGate` protected request lifecycle and
also serialized the entire stream-write transport mutation. This caused new
application write registration to wait behind packet construction, send-queue
mutation, completion, and retry work.

The first experiment removed that serialization and is rejected. It improved a
short c16 diagnostic but exposed connection-wide packet and send-state mutation
to concurrency; a c64 cell returned only 221 of 1,024 expected response bytes.
The rejected evidence is retained under
`C:\shared\temp\pllockcandidate\c16` and
`C:\shared\temp\pllockcandidate\c64`.

The accepted design keeps transport mutation serialized, but separates it from
request lifecycle. Pending actions use an exact-value `ConcurrentDictionary`,
per-request completion ownership arbitrates cancellation and terminal cleanup,
a processing gate retains the original packet/send-state serialization, and a
separate retry-queue gate protects retry bookkeeping. Exact removal prevents a
late cancellation or terminal path from completing a reused pooled source.

Matched shared-host multiplex results used exact payload validation and five
usable repetitions per side unless noted:

| Connections | Baseline | Candidate | Throughput delta | Baseline range | Candidate range | p95 delta | p99 delta |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 6.59 MiB/s | 6.71 MiB/s | +1.8% | 6% | 3% | -3.2% | -4.4% |
| 4 | 18.56 MiB/s | 19.05 MiB/s | +2.7% | 9% | 8% | -5.5% | +2.8% |
| 16 | 25.73 MiB/s | 25.14 MiB/s | -2.3% | 8% | 8% | -4.7% | -4.0% |
| 64 | 14.14 MiB/s | 15.89 MiB/s | +12.4% | 37% | 21% | -26.5% | -29.2% |

One baseline c16 cell and one candidate c64 cell independently produced the
same 221-byte truncated response and were excluded from matched metrics. That
failure therefore predates this candidate and remains a correctness/reliability
backlog item. The c64 result is diagnostic because variance is high, but its
throughput and tail-latency direction is repeated across five exact matched
pairs. Evidence is retained under
`C:\shared\temp\pllockcandidate\v2-c1`, `v2-c4`, `v2-c16`, and `v2-c64`.

Five-repetition control medians under
`C:\shared\temp\pllockcandidate\v2-controls-corrected` stayed bounded:

| Control | Throughput delta | Candidate throughput range | p95 delta | p99 delta |
| --- | ---: | ---: | ---: | ---: |
| 16x1 MiB duplex | -0.5% | 7.8% | +1.0% | +1.0% |
| 64 KiB upload | -3.4% | 12.0% | +2.1% | -0.9% |
| 16,384x1 KiB fixed total | -1.5% | 1.5% | +2.2% | +5.8% |
| 256x64 KiB fixed total | -0.1% | 5.9% | +1.9% | -4.0% |

The 16 MiB upload control needs an explicit reliability caveat. Baseline cells
completed 10/10; candidate cells completed 8/10. Two adjacent candidate cells
sent the full payload but timed out with no successful request and no recent
network activity. The next five candidate-only supplements passed, producing
six consecutive candidate successes including the preceding matched cell. This
is the same load-tool timeout class retained from earlier shared-host upload
runs, and code review found no candidate path that changes single-stream packet
mutation semantics, but the result is not counted as a reliability improvement.

Wake-cycle diagnostics at c64 use ProtocolLab parser commit `68effd6`. The
candidate reduced async wakeups from 5,344 to 2,903 and increased mean work per
async wakeup from 2,048.6 to 2,401.6 while draining 425,522 of 425,802 stream
writes. Instrumented throughput rose from 9.80 to 12.79 MiB/s and p95 fell from
546.1 to 450.2 ms; these numbers are diagnostic only. Queue depth did not
improve: average shard peak rose from 496.5 to 510.6 and absolute peak from 561
to 569. Maximum outstanding pooled bytes were also similar at 7.09 versus 7.29
MiB. Inclusive CPU attribution showed request-registration monitor time falling
from 0.377% to 0.314%, but total slow-monitor time and thread-pool semaphore
waiting stayed flat at roughly 4.2% and 67.2%. Evidence is under
`C:\shared\temp\pllockcandidate\v2-diagnostics`.

Focused cancellation, completion-pool, and structural requirement tests passed
20/20, including a 128-write transition/cancellation race; the race passed ten
additional repetitions. The all-solution run passed 9,605 tests and skipped
five, with one DoQ excessive-load timing failure that passed ten consecutive
exact reruns. The tracked trace-analysis project was added to this worktree's
sparse checkout so the solution itself builds. No authentication, flow control,
congestion control, loss recovery, cancellation, disposal, or `ValueTask`
contract was weakened.

This is a bounded high-concurrency scheduling improvement, not the end of the
queue-depth diagnosis. The next tranche should explain the unchanged c64 queue
peaks and pooled-byte retention, and should isolate the pre-existing truncated
response and large-upload idle-timeout failures before attempting another
scheduler variant.
