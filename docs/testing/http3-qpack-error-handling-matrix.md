...
| Client-sent PUSH_PROMISE on request stream | RFC 9114 Section 7.2.5 | H3_FRAME_UNEXPECTED | Connection error | No | `RequestStream_ClientSentPushPromise_ThrowsFrameUnexpected` | Interop client can send PUSH_PROMISE on its request stream; fuzz request-stream frame placement by endpoint role. |
| Server PUSH_PROMISE before client MAX_PUSH_ID opt-in | RFC 9114 Sections 4.6, 7.2.5, 7.2.7 | H3_ID_ERROR | Connection error | No | `RequestStream_UnadvertisedPushPromise_ThrowsIdError` | Interop server can send PUSH_PROMISE before client advertises push capacity; fuzz push IDs when the max push ID is unset. |
