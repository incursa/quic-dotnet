...
| Application-send first-write selection and continuation | `IQuicApplicationSendTurnPlanner`; listener factory; runtime constructor | Null or `QuicCurrentApplicationSendTurnPlanner` preserves the static priority/sequence scheduler. Injected planners may select a legal first queued write and decide whether another turn is scheduled. | Connection actor at an existing queued-send planning boundary. Interface dispatch only when a planner exists. | A plan applies to one actor turn; stream ordering checks remain authoritative. | Yes, by per-connection injected planner in tests and benchmarks. | Stable policy seam. Any future policy must keep the current planner as baseline and must not repeat the rejected fixed quantum. |
