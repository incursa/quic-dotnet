...
| Buffer copy and coalescing | `StreamActionRequestCompletion`, `QuicApplicationSendQueue`, `QuicConnectionRuntime.Streams`, `QuicConnectionSendRuntime`, `QuicRetransmissionQueue`, `QuicConnectionStreamState`, endpoint hosts, `QuicBufferPool`, and `QuicReceiveBufferPool` | Application retry copies, raw queued data, formatted/combined payloads, protected packets, sent plaintext, retransmissions, and receive segments use distinct pooled owners and explicit release paths. The observe-only contract covers retry, raw queue, formatting, combination, sent-plaintext retention, path-migration retransmission clones, and receive-segment construction or capacity reuse with versioned connection-local records and fixed-field aggregation. New receive-segment and flow-control retry request owners carry compact lifetime tokens and emit one closed release after authoritative return. The application-send queue now maintains total retained count, capacity, and oldest enqueue time across every enqueue, replacement, removal, transfer, and clear. Endpoint batching can preserve a contiguous managed run or add platform-specific staging copies. | Multiple admission, packet-plan, protection, recovery, endpoint, and receive-retention boundaries. V1 and the maintained total queue snapshot add no read-side scan; queue-cause detail, sent-packet and retransmission snapshots may still scan. Copy-scope retained age, pool outstanding, terminal-release correlation outside receive segments and application-write retry requests, and platform staging remain missing or unobserved. | Yes for each current buffer lifetime: ownership, capacity, completion trigger, and return path remain fixed until terminal release. There is no policy transition. | Observation only. `legacy_current` is the sole real value; force and shadow identities remain null. | The reviewed ownership inventory and current release checkpoints are in `adaptive-runtime-stage2-buffer-ownership-copy-inventory.md` under `REQ-QUIC-CRT-0182` and `REQ-QUIC-CRT-0185`. Add the remaining owner release paths and a distinct conservative implementation before forcing. Do not silently call an existing mechanism a counterfactual policy. |
