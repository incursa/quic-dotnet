...
observation, buffer-copy v2, and post-service unified-evidence contracts
...
| Buffer copy and coalescing | `StreamActionRequestCompletion`, `QuicApplicationSendQueue`, `QuicConnectionRuntime.Streams`, `QuicConnectionSendRuntime`, `QuicRetransmissionQueue`, `QuicConnectionStreamState`, endpoint hosts, `QuicBufferPool`, and `QuicReceiveBufferPool` | Application retry copies, raw queued data, formatted/combined payloads, protected packets, sent plaintext, retransmissions, and receive segments use distinct pooled owners and explicit release paths. The first observe-only contract covers retry, raw queue, formatting, combination, and sent-plaintext retention with versioned connection-local records and fixed-field aggregation. The application-send queue now maintains total retained count, capacity, and oldest enqueue time across every enqueue, replacement, removal, transfer, and clear. Endpoint batching can preserve a contiguous managed run or add platform-specific staging copies. | Multiple admission, packet-plan, protection, recovery, endpoint, and receive-retention boundaries. V1 and the maintained total queue snapshot add no read-side scan; queue-cause detail, sent-packet and retransmission snapshots may still scan. Terminal release, retained age at copy scope, pool outstanding, retransmission clone, receive segment, and platform staging remain missing or unobserved. | Yes for each current buffer lifetime: ownership, capacity, completion trigger, and return path remain fixed until terminal release. There is no policy transition. | Observation only. `legacy_current` is the sole real value; force and shadow identities remain null. | The reviewed ownership inventory and v1 scope are in `adaptive-runtime-stage2-buffer-ownership-copy-inventory.md` under `REQ-QUIC-CRT-0182`. Add missing release/retention paths and a distinct conservative implementation before forcing. Do not silently call an existing mechanism a counterfactual policy. |
