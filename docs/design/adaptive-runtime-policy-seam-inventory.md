...
| Buffer copy and coalescing | `StreamActionRequestCompletion`, `QuicApplicationSendQueue`, `QuicConnectionRuntime.Streams`, `QuicConnectionSendRuntime`, `QuicRetransmissionQueue`, `QuicConnectionStreamState`, endpoint hosts, `QuicBufferPool`, and `QuicReceiveBufferPool` | Application writes, raw queued data, formatted/combined payloads, protected packets, sent plaintext, retransmissions, and receive segments use distinct pooled owners and explicit release paths. Endpoint batching can preserve a contiguous managed run or add platform-specific staging copies. | Multiple admission, packet-plan, protection, recovery, endpoint, and receive-retention boundaries. Process-wide pool metrics exist, but connection-attributable copy and exact-release evidence is incomplete; current diagnostic retention snapshots may scan collections. | Yes for each current buffer lifetime: ownership, capacity, completion trigger, and return path remain fixed until terminal release. There is no policy snapshot or transition. | No policy value. Only `legacy_current` is real. | The reviewed ownership inventory is in `adaptive-runtime-stage2-buffer-ownership-copy-inventory.md`. Add bounded connection-local observation and maintained retention counters before defining a distinct conservative value. Do not silently call an existing mechanism a counterfactual policy. |
