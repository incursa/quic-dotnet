...
| `oversized_write_admission_quantum` | At logical-write admission, `legacy_current` retains the dispatcher-plus-16-through-24-observer selector; `single_fragment` uses the conventional path; `bounded_multi_fragment` uses the accepted two-fragment path when the dispatcher guard permits it. | `legacy_current`, `single_fragment`, `bounded_multi_fragment` | Logical-write admission; immutable through fragmentation, continuation, retry, cancellation, disposal, terminal state, and completion. | Implemented: connection-local forcing, disabled/observe/shadow modes, versioned admission observation and decision, dispatcher safety fallback, exact logical-write outcome, listener copy, deterministic replay, adjacent-axis rejection, guarded sink, and force-legacy rollback. Still open: unified raw epoch export, permanent campaign control, BenchmarkDotNet cost evidence, full Release, and multi-host verification. |
