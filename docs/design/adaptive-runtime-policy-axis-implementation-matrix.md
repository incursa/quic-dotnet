...
pilot completed timing for exactly A0, A3, A4, and A7 with bounded aggregate
evidence and a directional-only result; performance acceptance, adaptive-rule
derivation, and active behavior remain unauthorized
...
activation proofs and their correctness-only interaction are independently
reviewed and passed. Both values remain correctness-eligible, forceable,
rollback-capable, and safe beneath their operation guards. The bounded offline
four-configured-cell campaign and its activation-qualified holdout extension
completed. The extension closed the prior holdout activation gap but did not
support a stable adaptive-selection rule. Its exact rule-promotion result is
`measurement_completed_no_stable_rule`; this is not an actuation or
correctness failure and does not authorize active behavior.
...
| `application_send_batch_formation` | 1 | existing eligible-prefix packing | `single_eligible` | yes | yes | yes | yes; fixed candidate/eligibility/applied/mechanism operation evidence plus per-behavior epoch counts and bytes | legal prefix and payload cap, priority, same-stream order, FIN, ownership, congestion, pacing, anti-amplification, flow control, packet/queue/buffer, lifecycle | yes | activation proof independently reviewed and passed; correctness eligibility retained; approved two-axis interaction and activation-qualified holdout extension complete; bounded four-cell admission rack pilot completed with a directional-only result | two-axis extension result `measurement_completed_no_stable_rule`; pilot cannot separate batch from buffer; performance acceptance, selector promotion, and active production behavior remain unauthorized | `1713b6a8`, `0f8797e0`, `cc1aaaf0`, `64c860a4`, `b1620359`, `b3c39539`, `e8c62e82`, `fc82651c` |
| `queued_send_burst_budget` | 1 | retained pre/post-confirmation static budget | `single_datagram` | yes | yes | yes | yes; operation-evidence v4 reviewed chain with actor-turn identity and catalog-derived behavior/outcome | congestion, pacing, anti-amplification, recovery, retransmission, handshake, packet, endpoint, flow control, queue/buffer, lifecycle rechecked per datagram | yes | independent review passed and exact proof promotion applied; the axis remains in its separate single-axis family | admission interaction, measurement, and active selection remain unauthorized | `fb520dd7`, `fd85f830`, `94b7391f`, `38408476` |
| `oversized_write_admission_quantum` | 1 | retained dispatcher plus bounded observer selector | reviewed `single_fragment`; blocked `bounded_multi_fragment` candidate | yes | yes | yes | yes; operation-evidence v4 chains distinguish the legacy-two and legacy-one activation bands; exact A0-A7 interaction evidence is retained | dispatcher availability, logical-write ownership, FIN, retry, recovery, congestion, pacing, anti-amplification, flow control, packet/queue/buffer, lifecycle, exactly-once completion | yes | `single_fragment` independently reviewed, promoted, and validated in all A4-A7 cells; bounded A0/A3/A4/A7 rack measurement completed with a directional-only result; `bounded_multi_fragment` remains unpromoted | `bounded_multi_fragment`: `shadow_recommendation_value_mismatch`; performance acceptance, rule derivation, and active selection remain unauthorized | `7d41e382`, `fd85f830`, `94b7391f`, `f1b0b320`, `0f2205d1`, `42ec5cc5`, `9644b197`, `fc82651c` |
...
| `buffer_copy_coalescing` | 2 | exact existing combined-send prefix construction | `memory_conservative`, lower-only two-source-segment cap | yes | yes | yes | yes; fixed candidate/eligibility/applied/mechanism evidence, decision-to-owner-to-release correlation, and per-behavior epoch counts and bytes | Stage 1 legal prefix, priority/order, FIN/reset/cancellation, flow control, stream capacity, packet size, congestion, pacing, anti-amplification, recovery, packet protection, owner transfer/return, terminal cleanup | yes | activation proof independently reviewed and passed; correctness eligibility and exact-release proof retained; approved two-axis interaction and holdout extension complete; bounded four-cell admission rack pilot completed with a directional-only result | two-axis extension result `measurement_completed_no_stable_rule`; pilot cannot separate batch from buffer; performance acceptance, selector promotion, and active production behavior remain unauthorized | `df8ee570`, `0f8797e0`, `cc1aaaf0`, `1e2aad3f`, `b1620359`, `b3c39539`, `e8c62e82`, `fc82651c` |
...
- The exact `send_admission_composition_correctness_v1` capability executed A0
  through A7 one cell at a time. All eight cells are
  `correctness_passed`; the family capability is
  `correctness_interaction_validated`.
- The capability is finite and manifest-bound. It does not authorize
  `bounded_multi_fragment`, nonlegacy queued burst, a fourth axis, a wildcard,
  performance acceptance, or active behavior.
- Factor-cell-space v3 binds exactly A0 through A7 to family catalog v5,
...
- An additive package-backed ProtocolLab rack pilot selects A0, A3, A4, and A7
  ...
  `isolated-pair` placement. The bounded run completed all four cells and eight
  repetitions with exact bytes, zero failures, zero timeouts, and no detected
