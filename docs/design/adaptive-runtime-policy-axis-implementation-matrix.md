...
Status: implementation-breadth checkpoint; bounded send-composition holdout
extension complete with `measurement_completed_no_stable_rule`; active
...
| `queued_send_burst_budget` | 1 | retained pre/post-confirmation static budget | `single_datagram` | yes | yes | yes | yes; operation-evidence v4 candidate chain with actor-turn identity and catalog-derived behavior/outcome | congestion, pacing, anti-amplification, recovery, retransmission, handshake, packet, endpoint, flow control, queue/buffer, lifecycle rechecked per datagram | yes | candidate actuation proof generated; external review pending | reviewed-proof metadata is intentionally absent; measurement and multi-axis execution remain blocked | `fb520dd7`, `fd85f830` |
| `oversized_write_admission_quantum` | 1 | retained dispatcher plus bounded observer selector | `single_fragment`; distinct `bounded_multi_fragment` candidate | yes | yes | yes | yes; two operation-evidence v4 candidate chains distinguish the legacy-two and legacy-one activation bands | dispatcher availability, logical-write ownership, FIN, retry, recovery, congestion, pacing, anti-amplification, flow control, packet/queue/buffer, lifecycle, exactly-once completion | yes | candidate actuation proofs generated for both nonlegacy values; external review pending | reviewed-proof metadata is intentionally absent; admission-family interaction and measurement remain blocked | `7d41e382`, `fd85f830` |
