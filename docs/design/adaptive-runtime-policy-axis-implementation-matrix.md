...
`connection_shard_placement` now owns exactly one registration-time choice.
`legacy_current` retains handle modulo shard count.
`bounded_power_of_two_choices` compares that shard with one deterministic
alternate using exactly two active-connection counters and uses the legacy
shard as its tie break. Registration failure and unregister restore the exact
counter, the route stores the immutable lifetime decision, and no migration,
topology input, queue scan, or threshold is introduced. The next independent
implementation axis is `application_datagram_batch_transport`.
