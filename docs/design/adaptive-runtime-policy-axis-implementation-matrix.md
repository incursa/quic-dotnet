...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `receive_credit_publication` | 0 foundation | retained read-dominant selector | `immediate` | yes | yes | yes | yes | flow-control progress, pending-credit flush, sticky application-write fallback, lifecycle | yes | accepted foundation | none for the retained internal seam; active selection remains unauthorized | `55cea560`, `6e408789` |
...
| `actor_work_quantum` | 2 | current connection-work-item dispatch to completion | none yet | foundation | no | no | actor-service epoch and continuation-assessment foundation | actor transition/effect ownership, exactly resumable work, progress, repost correctness, cross-connection service | no | continuation assessment focused proof clean | no reviewed cooperative-yield boundary owns an exactly resumable unit; a distinct forced value would currently be behaviorally fake or could split transition/effect ownership | `19f69274`, `36c5bad6`, `11bb8496`, `90aaf93b`, `CURRENT_CHECKPOINT` |
| `ready_stream_fairness` | 2 | current planner/queue order and same-stream serialization | none yet | partial | no | no | no axis record yet; bounded actor and send-path inputs are available | no same-stream reordering, priority authority, FIN/reset/cancellation, flow-control and recovery progress, bounded runnable inspection | no | inventory required | needs a bounded runnable-stream mechanism and real starvation/fairness outcome that does not require an unbounded stream scan | none |
| `buffer_copy_coalescing` | 2 | current copy, segment, coalesce, retain, clone, and reuse mechanisms | none yet | foundation | no | no | buffer-copy observation/epoch foundation; not yet an axis decision record | exact owner transfer/return, packet protection, retransmission retention, receive-segment lifetime, buffer/packet limits, terminal cleanup | no | observation and ownership producer proof clean | no unified plan-producing seam or distinct ownership-safe conservative value | `4aa4367f`, `086ab431`, `753cfb06`, `4791605d`, `a239a350`, `66943c96` |
...
`actor_work_quantum` forceable. It closes the current prerequisite slice and
records the exact unsafe boundary. The next work must either:

1. deliver a distinct forceable `ready_stream_fairness` seam with bounded
   runnable inspection and authoritative ordering/progress guards; or
2. if that seam proves dependent on the same unsafe cooperative-yield
   boundary, record that dependency and move directly to the independent
   `buffer_copy_coalescing` plan seam.

No further actor observation-only checkpoint is authorized before an
axis-level capability is delivered or the blocker above is resolved by a
reviewed design.
