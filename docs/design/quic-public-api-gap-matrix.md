...
The approved public facade is now in place, and the current slice includes narrow but real client-entry establishment, stream-entry, write/completion, a narrow `RESET_STREAM` / `STOP_SENDING` abort pair, and the stream-capacity callback boundary. The remaining gaps are behavioral rather than boundary-shape gaps: stream-close-driven capacity release, fuller client TLS/auth parity, combined `Abort(Both, ...)`, and the broader stream-management follow-ons.
...
4. Add stream-close-driven capacity release only when the stream-management path exists.
...
- The stream-capacity callback surface is now present on the supported loopback and active-loopback paths, but stream-close-driven capacity release is still deferred.
...
- Add stream-close-driven capacity release on top of the now-real stream-capacity callback boundary.
...
- Missing behavior: stream-close-driven capacity release, combined `Abort(Both, ...)`, and the broader abort-heavy follow-on.

### Recommended Next Slice

Promote stream-close-driven capacity release next, backed by the existing runtime, TLS bridge, and narrow stream-entry plus stream-capacity boundaries, so it can build on a real connection instead of a pending shell.
