...
internal sealed class QuicConnectionRuntimeHost : IAsyncDisposable, IDisposable
{
    ...
    public bool TryPostStreamCapacityRelease(QuicConnectionHandle handle)
    {
        if (Volatile.Read(ref disposed) != 0)
        {
            return false;
        ...
        QuicStreamType streamType)
    {
        ...
        return shards[route.ShardIndex].TryPostStreamOpen(handle, route.Runtime, requestId, streamType);
    }
        ...
        ReadOnlyMemory<byte> streamDataSuffix = default)
    {
        ...
        return shards[route.ShardIndex].TryPostStreamWrite(
            handle,
            route.Runtime,
            requestId,
            actionKind,
        ...
        QuicConnectionRuntimeSendDatagramBatchObserver? sendDatagramBatchObserver = null)
    {
        ...
        for (int index = 0; index < shards.Length; index++)
        {
            ...
            processingTasks[index] = shards[index].RunAsync(
                ...
                sendDatagramBatchObserver is null
                    ? null
                    : (handle, updates) => sendDatagramBatchObserver(handle, shardIndex, updates));
        }
