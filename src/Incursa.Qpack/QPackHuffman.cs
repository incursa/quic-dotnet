...
internal static class QPackHuffman
{
    ...
    private const int DecodeCacheSize = 16;
    private const int MaximumCachedEncodedLength = 64;
    private const int MaximumCachedDecodedLength = 128;
    ...
    [ThreadStatic]
    private static DecodeCache? threadDecodeCache;

    public static string Decode(ReadOnlySpan<byte> source, QPackErrorCode errorCode)
    {
        ulong cacheHash = 0;
        DecodeCache? cache = null;
        if (source.Length is > 0 and <= MaximumCachedEncodedLength)
        {
            cacheHash = ComputeHash(source);
            cache = threadDecodeCache;
            if (cache is not null && cache.TryGet(source, cacheHash, out string? cached))
            {
                return cached;
            }
        }
        ...
        try
        {
            ...
            string decodedValue = Encoding.Latin1.GetString(decoded[..decodedLength]);
            if (cacheHash != 0 && decodedValue.Length <= MaximumCachedDecodedLength)
            {
                cache ??= threadDecodeCache ??= new DecodeCache();
                cache.Store(source, cacheHash, decodedValue);
            }
    ...
    private sealed class DecodeCache
    {
        private readonly Entry[] entries = new Entry[DecodeCacheSize];
        private int nextIndex;

        public bool TryGet(ReadOnlySpan<byte> source, ulong hash, [NotNullWhen(true)] out string? value)
        {
            for (int index = 0; index < entries.Length; index++)
            {
                ...
                if (entry.Hash == hash
                    ...
                    && source.SequenceEqual(encoded)
                    && entry.Value is string cached)
                {
                    value = cached;
                    return true;
                }
        ...
        public void Store(ReadOnlySpan<byte> source, ulong hash, string value)
        {
            entries[nextIndex] = new Entry(source.ToArray(), hash, value);
            nextIndex = (nextIndex + 1) % entries.Length;
        }
    }

    private readonly record struct Entry(byte[]? Encoded, ulong Hash, string? Value);
