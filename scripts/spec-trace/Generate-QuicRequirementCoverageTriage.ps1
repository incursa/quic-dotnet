...
function Get-RequirementGapMappings {
    ...
    return @(
        ...
        [pscustomobject]@{
            Slug               = 'connect-udp'
            Summary            = 'RFC 9298 CONNECT-UDP requirements remain blocked for tunnel setup and live UDP lifecycle/forwarding after the URI Template, target-validation, HTTP/3 request-policy, datagram Context ID, vulnerable-target filtering, forwarding-policy, early-datagram, and HTTP/1.1 upgrade floors.'
            RequirementPrefixes = @()
            RequirementIds     = @(
                'REQ-QUIC-RFC9298-0001'
                'REQ-QUIC-RFC9298-0021'
                'REQ-QUIC-RFC9298-0033'
                New-RequirementIdRange -Rfc 'RFC9298' -Start 36 -End 52
            )
        },
        [pscustomobject]@{
