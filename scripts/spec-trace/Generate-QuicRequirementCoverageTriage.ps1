...
function Get-RequirementGapMappings {
    ...
    return @(
        ...
        [pscustomobject]@{
            Slug               = 'connect-udp'
            Summary            = 'RFC 9298 CONNECT-UDP requirements remain blocked for HTTP/1.1 upgrade behavior, tunnel setup, early-datagram handling, UDP lifecycle, forwarding, congestion, and ECN after the URI Template, target-validation, HTTP/3 request-policy, datagram Context ID, and vulnerable-target filtering floors.'
            RequirementPrefixes = @()
            RequirementIds     = @(
                ...
                New-RequirementIdRange -Rfc 'RFC9298' -Start 96 -End 115
            )
        },
        [pscustomobject]@{
