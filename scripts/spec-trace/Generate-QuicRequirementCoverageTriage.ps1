...
function Get-RequirementGapMappings {
    ...
    return @(
        ...
        [pscustomobject]@{
            ...
            Summary            = 'RFC 9484 CONNECT-IP/MASQUE requirements remain blocked after the implemented foundation, URI Template, and request-policy slices; remaining work covers tunnel setup, HTTP-specific request formats, target/ipproto validation, capsules, route/address assignment, datagram forwarding, and manageability policy.'
            RequirementPrefixes = @()
            RequirementIds     = @(
                New-RequirementIdRange -Rfc 'RFC9484' -Start 31 -End 40
                New-RequirementIdRange -Rfc 'RFC9484' -Start 42 -End 213
            )
        },
        [pscustomobject]@{
