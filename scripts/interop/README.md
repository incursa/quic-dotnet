...
- The explicit `connectionmigration-server-proof` profile runs only the server-role `connectionmigration` attempt against `picoquic` and `lsquic` on hosted Linux so live corroboration can be refreshed without widening the major-peer matrix. It remains advisory proof collection for the inventory cell and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
- The companion `connectionmigration-server-proof-blocked` profile keeps `quic-go`, `msquic`, and `neqo` visible as a blocked comparison lane. The `neqo` cell uses `ghcr.io/mozilla/neqo-qns:latest`. It remains advisory evidence collection and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
