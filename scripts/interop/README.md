...
- The companion `connectionmigration-server-proof-blocked` profile keeps `lsquic`, `quiche`, `quic-go`, `msquic`, `neqo`, and `aioquic` visible as a blocked comparison lane. The `neqo` cell uses `ghcr.io/mozilla/neqo-qns:latest`. It remains advisory evidence collection and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
