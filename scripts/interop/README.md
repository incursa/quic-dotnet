...
- The explicit `connectionmigration-server-proof` profile runs only the server-role `connectionmigration` attempt against `quic-go`, `msquic`, and `neqo` on hosted Linux so live corroboration can be refreshed without widening the major-peer matrix. The `neqo` cell uses `ghcr.io/mozilla/neqo-qns:latest`. It remains advisory proof collection for the inventory cell and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
