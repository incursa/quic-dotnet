...
The current supported/executed cells are `handshake`, `transfer`, `retry`, `multiconnect`, `versionnegotiation`, `keyupdate`, and `resumption`; the remaining documented non-HTTP/3 cells are surfaced as explicit inventory entries that are still red or blocked. `chacha20` remains blocked pending CipherSuitesPolicy support. `zerortt` remains blocked pending `early_data` ticket advertisement, server ZeroRtt packet opening, and early application-delivery proof; helper dispatch, client-side probe emission, or internal admission-gate proof is not support evidence. `resumption` is green only for the runner's TLS session-resumption cell with managed SSLKEYLOGFILE export proof; it does not imply HTTP/3, 0-RTT, anti-replay, or broader resumption API support.
