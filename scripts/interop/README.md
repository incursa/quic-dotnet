...
- The companion `connectionmigration-server-proof-blocked` profile keeps `picoquic`, `lsquic`, `neqo`, `quiche`, `quic-go`, `msquic`, and `aioquic` visible as a blocked comparison lane. It remains advisory evidence collection and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
