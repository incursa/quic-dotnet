...
- The explicit `connectionmigration-server-proof` profile runs the server-role `connectionmigration` attempt against `neqo` with `quinn` on hosted Linux so live corroboration can be refreshed without widening the major-peer matrix. It remains advisory proof collection for the inventory cell and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
- The companion `connectionmigration-server-proof-blocked` profile keeps `picoquic`, `ngtcp2`, `neqo`, `lsquic`, `quiche`, `quic-go`, `msquic`, and `aioquic` visible as a blocked comparison lane. The `picoquic` cell uses `privateoctopus/picoquic:latest`. It remains advisory evidence collection and does not imply `rebind-port`, `rebind-addr`, or `v2` support.
