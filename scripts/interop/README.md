...
The current supported/executed cells are `handshake`, `transfer`, `retry`, `multiconnect`, `versionnegotiation`, and `chacha20`; the remaining documented non-HTTP/3 cells are now surfaced as explicit inventory entries that are still red or blocked.
