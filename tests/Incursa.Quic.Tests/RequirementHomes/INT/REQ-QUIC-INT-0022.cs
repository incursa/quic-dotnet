...
public sealed class REQ_QUIC_INT_0022
{
    ...
    public void ConnectionMigrationServerProofProfileAddsDedicatedHostedCorroborationLane()
    {
        ...
        Assert.Contains("Mirror picoquic OCI wrapper image", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("Override picoquic runner image", workflow, StringComparison.Ordinal);
        Assert.Contains("implementations_quic.json", workflow, StringComparison.Ordinal);
        Assert.Contains("ghcr.io/incursa/quic-interop-picoquic:latest", workflow, StringComparison.Ordinal);
