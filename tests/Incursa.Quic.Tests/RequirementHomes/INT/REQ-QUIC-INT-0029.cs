...
public sealed class REQ_QUIC_INT_0029
{
    ...
    public void XquicResidualIsTraceOwnedBeforeRuntimePromotion()
    {
        ...
        Assert.Contains("debug-client-xquic-refresh/20260618-085552662-client-chrome", currentStatus, StringComparison.Ordinal);
        Assert.Contains("15,355 bytes", currentStatus, StringComparison.Ordinal);
        Assert.Contains("all three cells failed", currentStatus, StringComparison.Ordinal);
        Assert.Contains("debug-client-transfer-xquic-current-after-docker-cleanup", spec, StringComparison.Ordinal);
        Assert.Contains("debug-client-xquic-refresh/20260618-085552662-client-chrome", spec, StringComparison.Ordinal);
        Assert.Contains("debug-client-transfer-xquic-current-after-docker-cleanup", gapLedger, StringComparison.Ordinal);
        Assert.Contains("debug-client-xquic-refresh/20260618-085552662-client-chrome", gapLedger, StringComparison.Ordinal);
        Assert.Contains("all-upstream-streamdata-keyupdate-local", architecture, StringComparison.Ordinal);
        Assert.Contains("xquic-specific ingress, packet-opening, path-liveness, or peer pacing/drain investigation", spec, StringComparison.Ordinal);
        ...
        Assert.Contains("debug-client-xquic-refresh/20260618-085552662-client-chrome", verification, StringComparison.Ordinal);
        Assert.Contains("June 18 shared ceiling", verification, StringComparison.Ordinal);
