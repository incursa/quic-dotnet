...
public sealed class REQ_QUIC_RFC9000_S19P16_0004
{
    ...
    [Requirement("REQ-QUIC-RFC9000-S19P16-0005")]
    [Requirement("REQ-QUIC-RFC9000-S19P16-0006")]
    public void TryParseRetireConnectionIdFrame_UsesBoundarySequenceNumberEncodings(ulong sequenceNumber)
    {
        QuicRetireConnectionIdFrame frame = new(sequenceNumber);
        byte[] encoded = QuicFrameTestData.BuildRetireConnectionIdFrame(frame);

        Assert.Equal((byte)0x19, encoded[0]);

        int expectedLength = sequenceNumber < 64 ? 2 : 3;
