...
public sealed class REQ_QUIC_RFC9000_S22P4_0004
{
    ...
    public void RequirementStatement_UsesAShortMnemonicFrameTypeName()
    {
        ...
        JsonElement requirement = document.RootElement
            ...
            .EnumerateArray()
            .Single(entry => entry.GetProperty("id").GetString() == "REQ-QUIC-RFC9000-S22P4-0004");

        Assert.Equal("Define Frame Type Name", requirement.GetProperty("title").GetString());
        Assert.Equal(
            "The Frame Type Name field MUST be a short mnemonic for the frame type.",
            requirement.GetProperty("statement").GetString());
        Assert.Equal("RFC 9000 §22.4 RFC9000-S22.4-B5-P0-S1", requirement.GetProperty("trace").GetProperty("upstream_refs")[0].GetString());
    }
