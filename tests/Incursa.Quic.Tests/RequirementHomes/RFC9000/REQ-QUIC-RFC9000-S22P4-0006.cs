...
public sealed class REQ_QUIC_RFC9000_S22P4_0006
{
    ...
    public void RequirementStatement_DescribesFrameFieldFormatAndSemantics()
    {
        ...
        JsonElement requirement = document.RootElement
            ...
            .EnumerateArray()
            .Single(entry => entry.GetProperty("id").GetString() == "REQ-QUIC-RFC9000-S22P4-0006");

        Assert.Equal("Describe frame field semantics in specifications", requirement.GetProperty("title").GetString());
        Assert.Equal(
        ...
        Assert.Equal("RFC 9000 §22.4 RFC9000-S22.4-B6-P4-S3", requirement.GetProperty("trace").GetProperty("upstream_refs")[0].GetString());
    }
