...
public sealed class REQ_QUIC_RFC9000_S19P16_0007
{
    ...
    public void RetireConnectionIdFrame_RejectsTheMaximumSequenceBoundary()
    {
        ...
        Assert.True(runtime.Transition(
            new QuicConnectionConnectionIdIssuedEvent(
                ObservedAtTicks: 0,
                ConnectionId: QuicVariableLengthInteger.MaxValue - 1,
                StatelessResetToken: statelessResetToken),
            nowTicks: 0).StateChanged);

        byte[] retirePayload = QuicFrameTestData.BuildRetireConnectionIdFrame(new QuicRetireConnectionIdFrame(QuicVariableLengthInteger.MaxValue));
        QuicConnectionTransitionResult result = QuicS19P16RetireConnectionIdTestSupport.TransitionOneRttPacket(
            runtime,
            runtime.ActivePath!.Value.Identity,
            QuicS17P2P3TestSupport.PacketConnectionId,
